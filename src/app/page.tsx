...
export default function HomePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 pb-28 space-y-6">
      {/* 1. CLEAN HERO CAROUSEL (SCREENSHOT 2 STYLE) */}
      <HeroCarousel items={top10India} defaultType="movie" />

      {/* 2. CONTINUE WATCHING (FIXED POSTERS) */}
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-1 px-1">
            {continueWatching.map((item, idx) => (
              <Link
                ...
                className="flex-none snap-start w-52 sm:w-60 p-2.5 rounded-2xl bg-[#0e0e14]/85 border border-white/10 hover:border-white/25 transition group flex items-center gap-2.5 shadow-md"
              >
                    ...
                    <Image
                      src={
                        item.posterPath.startsWith("http")
                          ? item.posterPath
                          : `${IMAGE_BASE}/w185${item.posterPath}`
                      }
                      alt={item.title}
                      fill
                      unoptimized
