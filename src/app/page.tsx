...
import { Sparkles, Play, Film, Volume2, Star, History, ArrowRight } from "lucide-react";
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // 1. Load Continue Watching from playback history
      const savedHistory = playbackHistory.getAll();
      if (savedHistory.length > 0) {
        setContinueWatching(savedHistory);
      } else {
        // Fallback demo in-progress items so the shelf is immediately active
        setContinueWatching([
          {
            id: "demo_1",
            tmdbId: 976573,
            title: "Jawan",
            type: "movie",
            posterPath: "/jCdqvdhpj340M1c6R3w2P6uM3wK.jpg",
            currentTime: 2400,
            duration: 9800,
            progressPercent: 24,
          ...
          },
          {
            ...
            season: 1,
            episode: 2,
            posterPath: "/9PFonQ95165agq9uWjWn4U3X3v7.jpg",
            currentTime: 1200,
            duration: 3200,
            progressPercent: 38,
            lastWatched: Date.now(),
    ...
    loadData();
    window.addEventListener("spectra_playback_updated", () => {
      setContinueWatching(playbackHistory.getAll());
    });
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 pb-28 space-y-5">
      ...
      {/* 2. BENEATH CAROUSEL: CONTINUE WATCHING SHELF */}
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1 px-1">
            {continueWatching.map((item, idx) => (
              <Link
                ...
                onClick={() => soundFx.playCinematicSwell()}
                className="flex-none w-52 sm:w-60 p-2 rounded-2xl bg-[#0e0e14]/80 border border-white/10 hover:border-white/25 transition group flex items-center gap-2.5 shadow-md"
              >
